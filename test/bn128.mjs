...
import { buildBn128, BigBuffer, utils } from '../dist/main.js';
const log2 = utils.log2;

await test("bn128", async function () {
    ...
    await it("It shoud do an inverse FFT in G1", async () => {
        const Fr = bn128.Fr;
        const G1 = bn128.G1;
    ...
    await it("It shoud do a big FFT/IFFT in Fr", async () => {
        const Fr = bn128.Fr;
...
//            console.log(Fr.toString(Ainv[i]));
    ...
    await it("It shoud do a big FFTExt/IFFTExt in Fr", async () => {
        const Fr = bn128.Fr;
        const N = 16;
...
//            console.log(Fr.toString(Ainv[i]));
    ...
    await it("It shoud do a big FFT/IFFT in G1", async () => {
        const Fr = bn128.Fr;
        const G1 = bn128.G1;
        const N = 512;
    ...
    await it("It shoud do a big FFT/IFFT in G1 ext", async () => {
        const Fr = bn128.Fr;
        const G1 = bn128.G1;
        const N = 1<<13;
    ...
    await it("It shoud do Multiexp", async () => {
        const Fr = bn128.Fr;
        const G1 = bn128.G1;
        const N = 1 << 10;
